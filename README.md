...
**Queries.** Generated by default. To use your own, pass `queries="queries.jsonl"` with `id` and `text` fields, a `.txt` with one query per line, or a list of strings. For an MTEB task, `queries="original"` uses the dataset's own queries.

**Task description.** One sentence on what counts as a good result, for example `"Given a claim, find documents that refute it"`. Both the generator and the judge see it. For an MTEB task it defaults to the task's own prompt.
...
**Other arguments.** `top_k` (documents judged per query, default 10), `seed`, `filter_queries` (LLM quality filter and deduplication, on by default), `batch_size` for encoding, `workers` for concurrent LLM calls.
